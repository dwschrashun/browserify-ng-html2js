...
function ngHtml2jsify(opts) {
  ...
  return function (file) {
    ...
    function end(content) {
      var fileName = opts.baseDir || opts.stripPathBefore ? file.replace(path.join(opts.rootLocation, opts.baseDir), '').replace(/\\/g, '/') : file.match(fileMatching)[1];
      if (opts.stripPathBefore) {

        if (opts.stripPathBefore instanceof RegExp || opts.stripPathBefore.match(isItARegex)) {
          var stripPathsRegex = opts.stripPathBefore instanceof RegExp ? opts.stripPathBefore : new RegExp (opts.stripPathBefore, "g");
          fileName = fileName.replace(stripPathsRegex, '');
        } else {
